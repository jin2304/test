...
- 파이썬의 Selenium 라이브러리와 ChromeDriver를 이용하여 웹 크롤링을 수행하여 '광고성 리게시글' 5346개와 '일반 리뷰 게시글' 4146개 게시글 해시태그와 게시글 텍스트 데이터를 수집하였음.
- 추가적으로 오픈 소스 플랫폼 GitHub에서 본 프로젝트와 관련된 인스타그램 게시글 데이터 'label_0.csv' 파일에서 38,288개 데이터를 수집하여 본 프로젝트에 맞게 재가공 하여 활용하였음. 이 과정을 통해 크롤링 인스타그램 데이터 수집을 보완하고, 데이터의 양과 다양성을 확보할 수 있었음. 


1) 웹 크롤링 방법
광고성 리뷰 게시글을 수집하기 위해, 웹 크롤러를 활용하여 인스타그램에서 주로 광고성 리뷰 게시물에 사용되는 #광고, #광고입니다, #협찬, #협찬광고, #협찬리뷰, #협찬제품, #제품제공 등의 해시태그를 검색후, 각 게시글에서 해시태그와 텍스트를 추출하여 데이터를 수집하였다. 수집된 광고성 게시글은 최종적으로 '광고.xlsx' 엑셀파일에 저장하였으며 0열에는 게시글 해시태그, 1열에는 게시글 텍스트가 기록되도록 설정하였다.

일반 리뷰 게시글을 수집하기 위해, 웹 크롤러를 활용하여 인스타그램에서 주로 일반 게시물에 사용되는 #내돈내산, #내돈내산후기, #광고아님, #협찬아님, #찐리뷰, #찐리뷰어 등의 해시태그를 검색후, 각 게시글에서 게시글의 해시태그와 텍스트 추출하여 데이터를 수집하였다. 수집된 광고성 게시글은 최종적으로 '일반.xlsx' 엑셀파일에 저장하였으며 0열에는 게시글 해시태그, 1열에는 게시글 텍스트가 기록되도록 설정하였다.
