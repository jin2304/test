...
## 계층형 구조 vs 도메인형 구조
 ...
 #### 1. 계층형 구조(Layered Architecture)
   ...
   - 장점: 전체적인 구조를 빠르게 파악할 수 있고, 규모가 적은 프로젝트나 개인 프로젝트에서는 효율적이다.
   - 단점: 규모가 큰 프로젝트에서는 한 패키지에 클래스들이 많이 생기고, 팀 프로젝트 시 팀원들이 같은 패키지를 사용한다. 또한 특정 도메인에 관련된 기능을 모아서 찾기가 힘들다.
  ...
  ![image](https://github.com/user-attachments/assets/efb72f95-6813-4566-a9b6-4f8223145983)
   ...
   - 장점: 특정 도메인의 기능을 모아 보기가 쉬우며, 도메인 단위의 개발 및 유지보수가 좋다. 또한 도메인별 결합도가 낮아지며, 팀 프로젝트에서는 역할을 도메인 단위로 나누어 별도의 패기지에서 개발할 수 있어 협업이 효율적이다.
   - 단점: 도메인간 Controller, Service, Repository 등의 구조가 반복된다.


결론: MSA에서는 각 도메인이 독립된 서비스로 운영되므로, 도메인별로 패키지를 구성하는 도메인형 구조(DDD 기반 설계)를 적용해야함.
...
![image](https://github.com/user-attachments/assets/b8120960-eb22-4f99-8e54-34f666af120f)
...
## 인프라 계층(infrastructure 계층) 
 - MSA 구조에서는 각 서비스들이 분리되어 있으므로, 외부 시스템과의 통신을 통해 데이터를 주고받아야 한다.
 - 때문에 이를 효율적으로 관리 및 책임을 분리하기 위해, 외부 서비스과의 통신 및 상호작용을 담당하는 계층을 별도로 분리하는 것이 필요하다.
 - Infrastructure 계층은 외부 서비스(Order, Product 등)와의 네트워크 통신, 데이터베이스 접근, 메시징 시스템 등과 같은 외부 시스템과 소통하는 인터페이스 역할과 기술적인 관심사를 분리하는 계층.
...
![이미지개요](https://github.com/user-attachments/assets/4dbe55b7-00c9-4184-8861-66733ba124e7)
...
<table align="center">
 ...
 <tr>
  <td align="left"><b>
   대학생 A 씨(21)는 일반인 후기가 많은 다이어트 보조제 광고를 보고 구매해서 복용한 뒤 복통을 겪었다며 해당 광고 게시글에 '먹고 나서 배가 아팠다'라는 댓글을 달았지만 얼마 뒤 게시물이 삭제된 걸 목격했다고 말했다. <br> 이어서 복통에 대한 댓글을 작성할 때 '나만 이 제품과 안맞아서 그런가'라는 생각이 들어 업체에 연락을 하지 않았다고 덧붙였다.</b>
  </td>
 </tr>
</table>
...
<table align="center">
 ...
 <tr>
  <td align="left"><b>
   직장인 B씨(21)는 평소 즐겨보던 유튜버가 액상차를 먹고 붓기제거와 독소배출 효과를 봤다는 영상을 보고 해당 제품을 구매했다. 하지만 이는 전혀 효과가 없었고, 이후 인터넷 기사에서 허위, 과장 광고로 적발된 제품에 해당 제품이 있는 것을 확인하고 배신감이 들었다고 전했다.</b>
  </td>
 </tr>
</table>
...
#### <div align="center">[ 표 2 ] 광고성/일반 리뷰 게시글 필터링 목록</div>
...
#### <div align="center"> [ 표 3 ] 모델 학습 결과 </div>
...
#### <div align="center">[ 표 4 ] 광고성 단어 상위 20개</div>
...
![image](https://github.com/user-attachments/assets/80f6705e-9134-4859-b01a-94eaaf38b441)
